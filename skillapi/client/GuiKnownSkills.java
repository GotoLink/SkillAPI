...
public class GuiKnownSkills extends GuiScreen {
	...
	public GuiKnownSkills(PlayerSkills player) {
		...
		for (int i=0;i<skillKeys.length;i++){
			skillKeys[i] = Keyboard.getKeyName(SkillAPIClientProxy.skillKeyBindings[i].keyCode);
		}
	}
	...
	private void drawGUIBackground() {
		...
		fontRenderer.drawStringWithShadow("Known Skills", width/2-73, height/2-61, 0xFCFC80);
		for (int i=0;i<skillKeys.length;i++)
			fontRenderer.drawString(skillKeys[i], width/2-90, (height/2-40)+(21*i), 0xE2E2E9);
	}
	...
	private void drawSkillInfo(Skill skill,int i, int x, int y) {
		if(skill!=null){
	        ...
	        GL11.glScalef(16F, 16F, 1F);
			fontRenderer.drawStringWithShadow(skill.getName(), x+20, y, skill.getNameColour());
	...
	private void drawSkillStats(Skill skill, int x, int y) {
		if(skill.getManaCost(player)>0) {
			...
			fontRenderer.drawString(String.valueOf(skill.getManaCost(player)), x+10, y+1, 0xCFFFDF);
			x+=25;
		}
		if(skill.getChargeupTime(player)>0) {
			...
			fontRenderer.drawString(String.valueOf(skill.getChargeupTime(player)), x+10, y+1, 0xCFFFDF);
			x+=34;
		}
		if(skill.getCooldownTime(player)>0) {
			...
			fontRenderer.drawString(String.valueOf(skill.getCooldownTime(player)), x+10, y+1, 0xCFFFDF);
			x+=34;
		}
		if(skill.getDuration(player)>0) {
			...
			fontRenderer.drawString(String.valueOf(skill.getDuration(player)), x+10, y+1, 0xCFFFDF);
		}
	}
	...
	private void drawToolTip(Skill skill, int mouseX, int mouseY) {
		if(skill!=null){
			...
			GL11.glDisable(3042 /*GL_BLEND*/);
			mc.fontRenderer.drawStringWithShadow(skill.getType(), mouseX+5, mouseY+5, 0xE0BC38);
			for (int i=0;i<desc.length;i++){
				mc.fontRenderer.drawString(desc[i], mouseX+5, mouseY+15+(9*i), 0x8FA8FF);
			}
		}
	}
	...
	private void sendSkillUpdate(int i, String skill) {
		PacketDispatcher.sendPacketToServer(new UpdateSkillPacket(player.entityId, i, skill).getPacket());
	}
