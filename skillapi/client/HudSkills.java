...
public class HudSkills {
	...
	private void drawChargeup(int scaledWidth, int scaledHeight) {
		if(player.chargingSkill!=null) {
			...
			game.ingameGUI.drawTexturedModalRect((scaledWidth-100)/2, scaledHeight-69, 0, 144, (int) (player.chargingSkill.charge*104/(player.chargingSkill.getChargeupTime(game.thePlayer)*SkillTickHandler.mult)), 8);
			game.fontRenderer.drawStringWithShadow(player.chargingSkill.getName(), (scaledWidth-game.fontRenderer.getStringWidth(player.chargingSkill.getName()))/2, scaledHeight-69, player.chargingSkill.getNameColour());
		}
	}
