...
public class EntityShockWave extends Entity {
	...
	private void spawnParticles(int angle) {
		FMLClientHandler.instance().getClient().effectRenderer.addEffect(new EntityBrightReddustFX(worldObj, posX+radius*Math.cos(Math.toRadians(rotationYaw))*Math.cos(Math.toRadians(rotationPitch))*Math.cos(Math.toRadians(angle)), posY+radius*Math.sin(Math.toRadians(angle)), posZ+radius*Math.sin(Math.toRadians(rotationYaw))*Math.cos(Math.toRadians(rotationPitch))*Math.cos(Math.toRadians(angle)), 1, 0,191,255));
	}
            ...
            for (int i = 0; i < list.size(); ++i){
                ...
                if (entity.canBePushed()){
                    ...
                    if (d2 >= 0.01D){
                        ...
                        d0 *= d3*(double)(1.0F - entity.entityCollisionReduction);
                        d1 *= d3*(double)(1.0F - entity.entityCollisionReduction);
